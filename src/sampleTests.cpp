...
TEST(environmentTest, passingTest) { EXPECT_EQ(true, true); }
...
TEST_F(Fixture, testUsingFixture) { EXPECT_EQ(sth, 7); }
...
struct FreeMock : public FreeInterface {
  ...
  // MOCK_CONST_METHOD2(methodName, ReturnType(ArgType1, ArgType2)); //Ye olde
  // way
};
...
INSTANTIATE_TEST_SUITE_P(ParametricInstatiation, ParametricTests,
...
TEST(ComplicatedTest, checkXAndYEqual0) {
  ...
  EXPECT_THAT(
      coords,
      testing::Field("location", &Coords6D::location,
